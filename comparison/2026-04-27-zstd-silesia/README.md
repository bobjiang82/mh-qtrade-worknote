...
> 注意：silesia_shuf.tar 只有 211 MB。L19 默认 8MB 窗口 → 仅 约26 个独立窗口，对单文件多线程压缩 zstd 给到大约 T=8 就饱和（下面数据可见）。这是数据量限制，不是硬件。
...
- **高 level (L19/L22) 压缩 6979P 仍快 约20%**：4.33 vs 3.58 MB/s @ L19，3.12 vs 2.53 @ L22。
...
- **T=8 即饱和** (约15 MB/s)。原因：silesia_shuf.tar 211 MB 下 8MB 窗口 ≈ 26 块，单文件 worker 拿不到更多任务。再加线程没收益，跟硬件无关。
- 6979P 在 T≥8 后的小幅退化（13-14 MB/s）比 9965 的稳定 约15.5 略差，可能是 Xeon 跨 NUMA 调度成本，但 silesia 数据量本身太小，结论强度不高。

## 结论

1. **Ratio 两机一致 (约4.04 @ L22)**：固定语料保证了真正可比性。
2. **L19/L22 单核压缩 6979P 快 约20%**（真实文本数据上）。
3. **解压速度 9965 略快 约8%**（L19）。
...
- 6979P 测试时仍有其他负载占用 约408 GiB 内存，CPU 噪声偏向使 6979P 略低估。
